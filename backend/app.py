...
def analyze_with_gpt4(text, instructions):
    """✅ Uses GPT-4 API to analyze text with improved response validation."""
    try:
        messages = [
        ...
        logging.error(f"❌ OpenAI API Status Error: {str(e)}")
        return "Error: OpenAI API returned an invalid status."
...
def parse_redlined_text(text):
    """✅ Improved method to detect biased words correctly."""
    try:
        logging.debug(f"🔍 GPT-4 Redlining Response: {text}")  # ✅ Debugging log

        biased_words = []
        neutral_alternatives = []

        # ✅ More flexible detection using regex
...
def analyze():
    """✅ Analyze text for bias and return results using GPT-4."""
    try:
        ...
        # ✅ Improve Bias Score Calculation
        bias_result = analyze_with_gpt4(
            text,
            "Analyze the bias in this article and return ONLY a number between 0 and 100. "
            "A higher number means more bias."
        )
        bias_score = extract_number(bias_result)
        if bias_score is None:
            return jsonify({"error": "Bias score could not be determined."}), 400

        # ✅ Improve Rewriting for Neutrality
        ...
        # ✅ Improve Bias Redlining Detection
        redlined_text = analyze_with_gpt4(
            ...
            "Do NOT add explanations, just return the formatted list."
        )
...
def source_check():
    ...
    try:
        ...
        logging.error(f"❌ Error in /source_check: {str(e)}")
        return jsonify({"error": "Internal Server Error"}), 500  # ✅ Fixed syntax error
